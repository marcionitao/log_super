...
def read_users(limit: int = 10, offset: int = 0, session=Depends(get_session), current_user=Depends(get_current_user)):
...
def update_user(user_id: int, user: UserSchema, session=Depends(get_session), current_user=Depends(get_current_user)):
    ...
    current_user.password = get_password_hash(user.password)  # encriptar hash
